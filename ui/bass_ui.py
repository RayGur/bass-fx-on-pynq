...
class Knob(tk.Canvas):
    ...
    def _draw(self):
        ...
        self.create_line(cx, cy, px, py, fill=C_KNOB_PTR, width=2, capstyle=tk.ROUND)

        # Centre dot
        self.create_oval(cx - 2, cy - 2, cx + 2, cy + 2, fill=C_DIM, outline="")

        # Label above
        self.create_text(
            cx, 2, text=self.label, fill=C_DIM, font=("Arial", 7), anchor="n"
        )
...
class App:
    ...
    def _build_topbar(self):
        ...
        self.conn_btn = self._mkbtn(
            top, "Connect", self._connect, bg="#1e2e1e", fg="#66cc66"
        )
        self.conn_btn.pack(side=tk.LEFT, padx=(0, 4))

        self.start_btn = self._mkbtn(
            top,
            "▶ Start FX",
            self._start_fx,
            bg="#1a2a3a",
        ...
        self.stop_btn = self._mkbtn(
            top, "■ Stop", self._stop_fx, bg="#2a1a1a", fg="#ff6644", state=tk.DISABLED
        )
        ...
        tk.Label(
            top, textvariable=self.sw_var, bg=C_PANEL, fg=C_DIM, font=("Courier", 8)
        ).pack(side=tk.RIGHT, padx=6)

        self.status_var = tk.StringVar(value="● Disconnected")
        self.status_lbl = tk.Label(
            top, textvariable=self.status_var, bg=C_PANEL, fg=C_DIM, font=("Courier", 9)
        )
    ...
    def _pedal_frame(self, parent, title, color):
        ...
        tk.Label(
            inner, text=title, bg=C_PEDAL, fg=color, font=("Arial", 14, "bold")
        ).pack(pady=(0, 4))
        return inner

    def _led(self, parent, var):
        c = tk.Canvas(parent, width=18, height=18, bg=C_PEDAL, highlightthickness=0)
        c.pack(pady=2)

        def redraw(*_):
            c.delete("all")
            fill = C_LED_ON if var.get() else C_LED_OFF
            c.create_oval(1, 1, 17, 17, fill=fill, outline=C_LED_RIM, width=1)
    ...
    def _stomp(self, parent, var, cmd_name):
        ...
        def redraw(*_):
            ...
            # Outer ring
            c.create_oval(4, 4, 104, 104, fill="#1a1a1a", outline=C_STOMP_RIM, width=2)
            # Rubber pad
            c.create_oval(
                12, 12, 96, 96, fill=bg, outline="#666666" if on else "#444444", width=2
            )
            # Specular highlight
            c.create_arc(
                22,
            ...
            c.create_text(54, 54, text=txt, fill=col, font=("Arial", 13, "bold"))
    ...
    def _build_dist_pedal(self, parent):
        ...
        self.thr_knob = Knob(
            ...
            fmt="{:.2f}",
            on_change=lambda v: self.send_cmd(f"threshold {v:.4f}"),
        )
        ...
        self.gain_knob = Knob(
            ...
            DIST_PRESETS[0]["gain"],
            fmt="{:.0f}x",
            on_change=lambda v: self.send_cmd(f"gain {int(round(v))}"),
        )
        self.gain_knob.pack(side=tk.LEFT, padx=10)

        tk.Label(f, text="PRESET", bg=C_PEDAL, fg=C_DIM, font=("Arial", 8)).pack(
            pady=(6, 0)
        )
        self.dist_preset_btns = self._preset_row(f, ["L", "H"], self._apply_dist_preset)
    ...
    def _build_wobble_pedal(self, parent):
        ...
        self.rate_knob = Knob(
            ...
            fmt="{:.1f}Hz",
            on_change=lambda v: self.send_cmd(f"lfo_rate {v:.2f}"),
        )
        ...
        self.depth_knob = Knob(
            ...
            fmt="{:.0f}%",
            on_change=lambda v: self.send_cmd(f"lfo_depth {int(round(v))}"),
        )
        self.depth_knob.pack(side=tk.LEFT, padx=10)

        tk.Label(f, text="WAH DEPTH", bg=C_PEDAL, fg=C_DIM, font=("Arial", 8)).pack(
            pady=(6, 0)
        )
        self.wah_btns = self._preset_row(f, ["A", "B", "C"], self._apply_wah_preset)

        tk.Label(f, text="SPEED PRESET", bg=C_PEDAL, fg=C_DIM, font=("Arial", 8)).pack(
            pady=(6, 0)
        )
    ...
    def _connect(self):
        ...
        def _worker():
            try:
                ...
                client.connect(
                    host, username=user, password=pwd, timeout=10, banner_timeout=15
                )
                self.ssh = client
                ...
                self.root.after(
                    0, lambda e=exc: self._set_status(f"● Error: {e}", "#ff4444")
                )
    ...
    def _on_connected(self):
        ...
        self.conn_btn.config(
            text="Disconnect", command=self._disconnect, bg="#2a1a1a", fg="#ff7755"
        )
            ...
            def _monitor_fx():
                try:
                    for line in iter(fx_stdout.readline, ""):
                        ...
                        if any(
                            kw in stripped
                            for kw in ("Codec ready", "audio loop", "Done.")
                        ):
                            codec_ready.set()
                except Exception:
                    pass
    ...
    def _set_dist_preset_btn(self, idx):
        ...
        for j, b in enumerate(self.dist_preset_btns):
            b.config(
                bg=C_BTN_ON if j == idx else C_BTN, fg=C_ACCENT if j == idx else C_DIM
            )
        p = DIST_PRESETS[idx]
        self.thr_knob.set_value(p["threshold"], notify=False)
        self.gain_knob.set_value(p["gain"], notify=False)
